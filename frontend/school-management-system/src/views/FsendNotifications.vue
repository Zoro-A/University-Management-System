...
async function fetchCourses() {
  ...
  try {
    const res = await axios.get(
      `${apiConfig.baseURL}/faculty/${encodeURIComponent(userId)}/courses`
    );
...
async function loadAllStudents() {
  ...
  try {
    for (const course of courses.value) {
      try {
        const res = await axios.get(
          `${apiConfig.baseURL}/faculty/${encodeURIComponent(
            userId
...
async function sendToSelected() {
  ...
  try {
    ...
    const tasks = selectedRecipients.value.map((id) => {
      ...
      return axios.post(apiConfig.url("faculty/notify"), payload);
    });
...
async function sendToAll() {
  ...
  try {
    ...
    await axios.post(apiConfig.url("faculty/notify"), payload);
